...
- **Dormant `iced_wgpu` lock entry.** The settings crate's iced `image-without-codecs` feature routes through `iced_renderer`, whose weak `iced_wgpu?/image` dependency puts `iced_wgpu` + a crates.io wgpu 27 stack into `Cargo.lock` without ever resolving them into a build graph (`cargo tree -i wgpu@27` is empty; any iced feature touching `iced_renderer` does this). The single-source invariant still holds for everything that compiles, and the cargo-deny ban (`deny-multiple-versions` on `wgpu`) fails CI loudly if a future change ever activates that entry. Accepted: lock-only noise beats forking iced.
